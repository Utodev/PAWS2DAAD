...
FUNCTION PeekSysmess(C:Word):Byte;
BEGIN
 if c<16384 Then Error(7);
 PeekSysmess := Spectrum^[c];
 if (PeekSysmess>=54) THEN PeekSysmess :=  PeekSysmess + 9; // Skip the DAAD additional SYSMESS
END;
...
PROCEDURE FillSystemMessages(Sysmess0: AnsiString);
BEGIN
 if (Pos('OSCURO', AnsiUppercase(Sysmess0))=0) THEN
 BEGIN
    PseudoWriteLn(FOut, '/54 "T" ;Letter for Tape');
    PseudoWriteLn(FOut, '/55 "D" ;Disc');
    PseudoWriteLn(FOut, '/56 "Drive not ready - press any key to retry.#n"');
    PseudoWriteLn(FOut, '/57 "I/O Error.#n"');
    PseudoWriteLn(FOut, '/58 "Disc or Directory may be full."');
    PseudoWriteLn(FOut, '/59 "Invalid filename."');
    PseudoWriteLn(FOut, '/60 "Type in name of file:"');
    PseudoWriteLn(FOut, '/61 "Start tape.#n"');
    PseudoWriteLn(FOut, '/62 "Tape or Disc?"');
 END
 ELSE
 BEGIN
  PseudoWriteLn(FOut, '/52 "No hay nada de eso en"');
  PseudoWriteLn(FOut, '/53 "Nada."');
  PseudoWriteLn(FOut, '/54 "C" ; Inicial de Cinta');
  PseudoWriteLn(FOut, '/55 "D" ; Disco');
  PseudoWriteLn(FOut, '/56 "Unidad no preparada. Pulsa una tecla para volver a intentarlo."');
  PseudoWriteLn(FOut, '/57 "Error de entrada/salida."');
  PseudoWriteLn(FOut, '/58 "El disco o el directorio puede estar lleno."');
  PseudoWriteLn(FOut, '/59 "Nombre de fichero no válido."');
  PseudoWriteLn(FOut, '/60 "Nombre del fichero:"');
  PseudoWriteLn(FOut, '/61 "Pon en marcha la cinta."');
  PseudoWriteLn(FOut, '/62 "¿Cinta o disco?"');
 END;
END;
 ...
 WHILE (I<=Finish) DO
  BEGIN
   ...
   WHILE (Spectrum^[Offs] <> (31 XOR $FF)) AND ((Offs<OffTab) or plain) DO
    ...
    END; 
    PseudoWriteLn(FOut,'"');
    if (Title = 'System Message') AND (I=0) THEN Sysmess0 :=  CleanString(TempStr);
    XPos:=0;
   if not plain and (Offs>=OffTab) then break;
   if (Title = 'System Message') AND (I=54) THEN 
    BEGIN
     FillSystemMessages(Sysmess0);
     I:=63;
    END; 
  END;
...
BEGIN
 ...
 WriteLn('A PAWS/Quill database extractor in DAAD Source File (DSF) format') ;
 WriteLn;
 ...
 WriteLn;
 WriteLn('Example :  ',Product,' RIPPER1.ZX RIPPER1.DSF -GAC -SZ80');
 WriteLn;
 WriteLn('Please remember adventures made with Quill/PAWs are copyright someone. Make sure you use this tool for your own games or ask permission from copytight holder.');
 Halt;
END;
    ...
    while (Peek(proptr)<>0) DO
      BEGIN
          ...
          BEGIN
            ...
            IF (TerminatorFound) THEN Write(FOut, '; PAWS2DAAD note: condact after terminator: ');
            n := n + 1;
            opcode:=MapQuillCondActToPAW(Peek(resptr),opcodetype);
            if (opcode <= 107) THEN
              BEGIN
                  AuxStr := condacts[opcode].condact;
                  IF AuxStr='DESC' THEN AuxSTR :=  'RESTART; PAWS2DAAD note: it was DESC';
                  IF AuxStr='INVEN' THEN AuxSTR := 'SYSMESS 9'#13#10'                   LISTAT  CARRIED'#13#10'                   SYSMESS 10'#13#10'                   LISTAT  WORN'#13#10'                   DONE ; PAWS2DAAD note: it was "INVEN"';
                  IF AuxStr='TURNS' THEN AuxSTR := '; PAWS2DAAD note: it was: "TURNS"';
                  IF AuxStr='TIMEOUT' THEN AuxSTR := 'HASAT 87; PAWS2DAAD note: it was: "TIMEOUT"';
                  IF AuxStr='CHARSET' THEN AuxSTR := '; PAWS2DAAD note: it was: "CHARSET"';
                  IF AuxStr='MODE' THEN AuxSTR := '; PAWS2DAAD note: it was: "TIMEOUT"';
                  IF AuxStr='GRAPHIC' THEN AuxSTR := '; PAWS2DAAD note: it was: "TIMEOUT"';
                  IF AuxStr='SAVE' THEN AuxSTR := 'SAVE 0 ; PAWS2DAAD note: it was just SAVE';
                  IF AuxStr='LOAD' THEN AuxSTR := 'LOAD 0 ; PAWS2DAAD note: it was just LOAD';;
                  IF AuxStr='PARSE' THEN AuxSTR := 'PARSE 0 ; PAWS2DAAD note: it was just PARSE';
                  IF AuxStr='PROMPT' THEN AuxSTR := '; PAWS2DAAD note: it was just PROMPT below'#13#10'            LET 42';
              ...
              if Opcode in terminatorOpcodes then TerminatorFound:=true;
          END;
          WriteLn(FOut);
      END;
      WriteLn(FOut);
